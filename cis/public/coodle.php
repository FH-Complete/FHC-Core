...
if($abgeschlossen)
{
	...
	echo '<br><br><div class="abgeschlossen"><b>'.$p->t('coodle/umfrageAbgeschlossen',array($datum->formatDatum($coodle_abgeschlossen->datum, 'd.m.Y').' '.$coodle_abgeschlossen->uhrzeit)).'</b></div>';
}
...
<div>';
		...
		foreach($coodle_termine->result as $termin)
		{
			...
			if($datum_colspan != $termin->datum && $termin->datum != '1900-01-01')
				echo "<th colspan='$colspan' class='".$class_auswahl."'>".$datum->formatDatum($termin->datum, 'd.m.Y')."</th>";
		...
		foreach($coodle_termine->result as $termin)
		{
			...
			if($termin->datum != '1900-01-01')
				echo "<th class='".$class_auswahl."'>".date('H:i',$time)."</th>";
			else 
				echo '<th style="background-color: #F2DEDE;" class="'.$class_auswahl.'">'.$p->t('coodle/keinTerminMoeglich').'</th>';
		}
		echo "</tr>";
			...
			else 
			{
				...
				foreach($coodle_termine->result as $termin)
				{
					...
					else 
					{
						...
						{
							if ($checked)
								echo '<td class="'.$class.'" align="center" '.$style.'><img src="../../skin/images/check_black.png" style="vertical-align: middle; height: 1em"></td>';
							else 
		...
		echo '<tr><td style="padding: 2px 4px;">Summe der Einträge</td>';		
		foreach($coodle_termine->result as $termin)
		{
			$countTermine = new coodle();
			$countTermine->countTermin($termin->coodle_termin_id);
			...
			if ($countTermine->anzahl == max($counter_arr))
				echo '<td style="padding: 2px 4px; background-color: #EAEBED;" align="center"><b>'.$countTermine->anzahl.'</b></td>';
			else 
				echo '<td style="padding: 2px 4px; background-color: #EAEBED;" align="center">'.$countTermine->anzahl.'</td>';
		}
		if($ersteller)
			echo '<td align="center">'.$p->t('coodle/keineAuswahl').'</td>';
		echo "</tr>";

		if($ersteller)
		{
			// buttons für auswahl des endgültigen termins
			echo '<tr><td>'.$p->t('coodle/auswahlEndtermin').'</td>';
			foreach($coodle_termine->result as $termin)
			{
				$checked=($termin->auswahl)?'checked':'';
				if ($termin->datum != '1900-01-01')
					echo '<td align="center"><input type="radio" onclick="showInfotext();" name="auswahl_termin" '.$checked.' '.$disabled.' value='.$termin->coodle_termin_id.'></td>';
				else
					echo '<td align="center"></td>';
			}
			echo '<td align="center"><input type="radio" onclick="hideInfotext();" name="auswahl_termin" '.$disabled.' value=""></td>';
			echo "</tr>";
		}
		if($ersteller && $abgeschlossen)
			echo '	<tr><td><a href="mailto:'.$mailadressen.'">Mail an alle schicken</a></td></tr>';
		echo '	<tr><td id="infotext" class="infotext" style="display: none" colspan="20">'.$p->t('coodle/auswahlHinweis').'</td></tr>
				<tr><td colspan="200"><input type="submit" value="'.$p->t('global/speichern').'" name="save" '.$disabled.'>';
