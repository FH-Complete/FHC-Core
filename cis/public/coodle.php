		...
		if ($auswahl != '')
		{
			...
			if ($coodle_help->datum < RES_TAGE_LEKTOR_BIS)
			{
				...
				foreach ($coodle_raum->result as $raum)
				{
					...
					{
						$message .= '<span class="error">' . $p->t('coodle/raumBelegt', array(
							$raum->ort_kurzbz
						)) . '</span><br>';
					}
				}
			...
			{
				$message .= '<span class="error">' . $p->t('coodle/raumNichtReserviert', array(
...
if ($abgeschlossen)
{
	...
	echo '<br><br><div class="abgeschlossen"><b>' . $p->t('coodle/umfrageAbgeschlossen', array(
...
echo '<tr><td class="normal">Summe der Einträge</td>';
foreach ($coodle_termine->result as $termin)
{
	$countTermine = new coodle();
	$countTermine->countTermin($termin->coodle_termin_id);
	...
	if ($countTermine->anzahl == max($counter_arr))
		...
		echo '<td class="footer">' . $countTermine->anzahl . '</td>';
}
if ($ersteller)
...
END:VCALENDAR";
	...
	if (count($coodle_ressource->result) > 0)
	{
		foreach ($coodle_ressource->result as $row)
		{
			...
			if ($mail->send())
			{
				$mailMessage .= $p->t('coodle/mailVersandtAn', array(
					$email
				)) . "<br>";
			}
		}
