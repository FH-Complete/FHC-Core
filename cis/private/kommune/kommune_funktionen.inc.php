...
function kommune_funk_eigene_wettbewerb($oWettbewerb)
{
	...
	$Wettbewerb=new komune_wettbewerbteam($oWettbewerb->sqlCONN,$oWettbewerb->userUID,$oWettbewerb->team_kurzbz,$oWettbewerb->wettbewerb_kurzbz);
	$Wettbewerb->setEncodingSQL($oWettbewerb->clientENCODE);
	$Wettbewerb->setSchemaSQL($oWettbewerb->sqlSCHEMA);
...
function kommune_funk_teams($oWettbewerb)		
{
	...
	$Wettbewerb=new komune_wettbewerbteam($oWettbewerb->sqlCONN,'','','');
   	$Wettbewerb->setEncodingSQL($oWettbewerb->clientENCODE);
	$Wettbewerb->setSchemaSQL($oWettbewerb->sqlSCHEMA);
  	...
  	for ($iTmpZehler=0;$iTmpZehler<count($oWettbewerb->TeamGesamt);$iTmpZehler++)
	{
		...
		{
			$paramURL=$_SERVER['PHP_SELF'].'?'.constKommuneParmSetWork.'='.constKommuneDisplayIMAGE.'&amp;timecheck'.time().'&amp;team_kurzbz='.$oWettbewerb->TeamGesamt[$iTmpZehler]["team_kurzbz"].'&amp;wettbewerb_kurzbz='.$oWettbewerb->TeamGesamt[$iTmpZehler]["wettbewerb_kurzbz"].(strlen($oWettbewerb->TeamGesamt[$iTmpZehler]["logo"])<2000?'&amp;heximg='.$oWettbewerb->TeamGesamt[$iTmpZehler]["logo"]:'');
...
function kommune_funk_anwenderteams($oWettbewerb)		
{
  	...
  	for ($iTmpZehler=0;$iTmpZehler<count($oWettbewerb->TeamAnwender);$iTmpZehler++)
	{
		...
		{
			$paramURL=$_SERVER['PHP_SELF'].'?'.constKommuneParmSetWork.'='.constKommuneDisplayIMAGE.'&amp;timecheck'.time().'&amp;team_kurzbz='.$oWettbewerb->TeamAnwender[$iTmpZehler]["team_kurzbz"].'&amp;wettbewerb_kurzbz='.$oWettbewerb->TeamAnwender[$iTmpZehler]["wettbewerb_kurzbz"].(strlen($oWettbewerb->TeamAnwender[$iTmpZehler]["logo"])<2000?'&amp;heximg='.$oWettbewerb->TeamAnwender[$iTmpZehler]["logo"]:'');
...
#		exit($arrWettbewerbTeam.Test($oWettbewerb->TeamAnwender));
	  	for ($iTmpZehler=0;$iTmpZehler<count($oWettbewerb->TeamAnwender);$iTmpZehler++)
		{
		...
		{
		$Wettbewerb=new komune_wettbewerbteam($oWettbewerb->sqlCONN,'',$arrWettbewerbTeam,'');
   		$Wettbewerb->setEncodingSQL($oWettbewerb->clientENCODE);
		$Wettbewerb->setSchemaSQL($oWettbewerb->sqlSCHEMA);
		if ($Wettbewerb->loadWettbewerbteam())
...
function kommune_funk_show_wettbewerbeinladungen_forderungstage($oWettbewerb)
{
	...
	$WettbewerbTermine= new komune_wettbewerbeinladungen($oWettbewerb->sqlCONN);
	$WettbewerbTermine->setEncodingSQL($oWettbewerb->clientENCODE);
	$WettbewerbTermine->setSchemaSQL($oWettbewerb->sqlSCHEMA);
	...
	$showHTML.='<table cellpadding="1" cellspacing="1" style="background-color: black;text-align:left;" summary="Wettbewerb Forderungstage">';    
		$showHTML.='<tr>
			...
			<td style="color:back;background-color: #DDDDDD;"> Pr&uuml;fung am Tagesdatum '.date("d.m.Y", mktime(0,0,0,date("m"),date("d"),date("y"))).'</td>			
			</tr>';	
...
#exit(Test($arrTempWettbewerbTermine));
# '.(!empty($arrTempWettbewerbTermine[$iTmpZehler]["farbe"])?' style="background : #'.$arrTempWettbewerbTermine[$iTmpZehler]["farbe"].';vertical-align: top;" ':'  style="vertical-align: top;" ').' 		
...
#		$showHTML.="<hr />".trim($arrTempWettbewerbTermine[$iTmpZehler]["team_gefordert"]).Test($oWettbewerb->TeamBenutzer[trim($arrTempWettbewerbTermine[$iTmpZehler]["team_gefordert"])][0]). "<hr />";

	}		   
       $showHTML.='</table>';
#	$showHTML.=Test($arrTempWettbewerbTermine).Test($oWettbewerb);
		$showHTML.='</fieldset>';
		return $showHTML;    	
   	return $showHTML;                            
}
...
// --- Wettbewerbe (EINLADUNGEN)                
	...
	$WettbewerbEinladung= new komune_wettbewerbeinladungen($oWettbewerb->sqlCONN);
	$WettbewerbEinladung->setEncodingSQL($oWettbewerb->clientENCODE);
	$WettbewerbEinladung->setSchemaSQL($oWettbewerb->sqlSCHEMA);
...
#exit(Test($arrTempWettbewerb[$iTmpZehler]))  ;
...
#	$showHTML.=Test($oWettbewerb->Spiele);
...
function kommune_funk_benutzerperson($userUID,$oWettbewerb)
{
	...
	if (trim($oWettbewerb->clientENCODE)!='LATIN9')
		$unicode=$oWettbewerb->clientENCODE;
	else
		$unicode=null;
	...
	if (!empty($pers->foto))
	{
		$paramURL=$_SERVER['PHP_SELF'].'?'.constKommuneParmSetWork.'='.constKommuneDisplayIMAGE.'&amp;timecheck'.time().'&amp;person_id='.$pers->person_id.(strlen($pers->foto)<2000?'&amp;heximg='.$pers->foto:'');
...
function kommune_funk_Statistik($oWettbewerb)
{
	...
	$Wettbewerb->InitWettbewerbteam();
    	$cTmpSQL="";
		$cTmpSQL.="SELECT * FROM ".$oWettbewerb->sqlSCHEMA.".tbl_wettbewerbteam where punkte>0";
    	$cTmpSQL.=" order by punkte desc OFFSET 0 LIMIT 3;";	
	...
	if (!$Wettbewerb->fetch_all($cTmpSQL)) 
		$oWettbewerb->Error[]=$Wettbewerb->getError();
	   	...
	   	$aPunkteSieger=$Wettbewerb->getResultSQL();
	...
	for ($iTmpZehler=0;$iTmpZehler<count($aPunkteSieger);$iTmpZehler++)
	{
		...
		$WettbewerbT=new komune_wettbewerbteam($oWettbewerb->sqlCONN,'',$aPunkteSieger[$iTmpZehler]['team_kurzbz'],$aPunkteSieger[$iTmpZehler]['wettbewerb_kurzbz']);
		$WettbewerbT->setEncodingSQL($oWettbewerb->clientENCODE);
		$WettbewerbT->setSchemaSQL($oWettbewerb->sqlSCHEMA);
...
// Gesamtuebersicht-------------------------------------------------------------------------------------------	
	...
	// Anzahl Teams Spieler je Wettbewerb
		    	$cTmpSQL="";
				$cTmpSQL.="SELECT count(distinct tbl_wettbewerbteam.team_kurzbz) as count_team_kurzbz FROM ".$oWettbewerb->sqlSCHEMA.".tbl_wettbewerbteam ";
			...
			$cTmpWettbewerbInfo=array();
		       $Wettbewerb->setResultSQL(null);
			if ($Wettbewerb->fetch_all($cTmpSQL)) 
			   	$cTmpWettbewerbInfo=$Wettbewerb->getResultSQL();
	...
	// Besten suchen		
		    	$cTmpSQL="";
				$cTmpSQL.="SELECT tbl_wettbewerbteam.wettbewerb_kurzbz,team_kurzbz,punkte,rang FROM ".$oWettbewerb->sqlSCHEMA.".tbl_wettbewerbteam ";
		   		$cTmpSQL.=" WHERE UPPER(tbl_wettbewerbteam.wettbewerb_kurzbz)=UPPER(E'".trim($oWettbewerb->Wettbewerb[$iTmpZehler]["wettbewerb_kurzbz"])."') ";	
		    	$cTmpSQL.=" order by rang ASC OFFSET 0 LIMIT 1 ;";	
			$arrWettbewerbTeams=array();
		       $Wettbewerb->setResultSQL(null);
			$cTmpBesteTeam=array();
			if ($Wettbewerb->fetch_all($cTmpSQL)) 
			   	$cTmpBesteTeam=$Wettbewerb->getResultSQL();
	...
	// Aktivsten suchen		
				...
				$cTmpSQL.="SELECT tbl_wettbewerbteam.wettbewerb_kurzbz,team_kurzbz,punkte,rang FROM ".$oWettbewerb->sqlSCHEMA.".tbl_wettbewerbteam ";
	   			$cTmpSQL.=" WHERE UPPER(tbl_wettbewerbteam.wettbewerb_kurzbz)=UPPER(E'".trim($oWettbewerb->Wettbewerb[$iTmpZehler]["wettbewerb_kurzbz"])."') ";	
	    		$cTmpSQL.=" order by punkte DESC OFFSET 0 LIMIT 1 ;";	
			$cTmpAktivsteTeam=array();
		       $Wettbewerb->setResultSQL(null);
			if ($Wettbewerb->fetch_all($cTmpSQL)) 
			   	$cTmpAktivsteTeam=$Wettbewerb->getResultSQL();
...
#echo " $empf, $betreff, $text, From: ".(defined('CAMPUS_NAME')?CAMPUS_NAME.' ':'').$oWettbewerb->wettbewerb_kurzbz.' '.$cTmpName." <$abs>" ;
#exit(Test($oWettbewerb));
...
*/
       ...
       {              
        ...
        // Plausib der UploadDaten      
              ...
              $sqlSELECT="SET search_path TO kommune;"; 
