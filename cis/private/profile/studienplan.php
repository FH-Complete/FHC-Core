...
echo '
	...
	{
		eval(targ + ".location=\'" + selObj.options[selObj.selectedIndex].value + "'.$getString.'\'");
...
if(count($prestudent->result) > 0)
{
	...
	foreach ($prestudent->result as $pres)
	{
		...
		echo "<OPTION ".(isset($prestudent_id) && $prestudent_id == $pres->prestudent_id ? "selected" : "")." value='studienplan.php?prestudent_id=".$pres->prestudent_id.(count($getParam)>0?"&":"")."'>$studiengang->bezeichnung</OPTION>";
	}
	echo "</SELECT>";
			...
			elseif(count($kompatibleLVs) > 0)
			{
		                  ...
		                  while(!$found && $i < count($kompatibleLVs))
		                  {
		                      foreach($kompatibleLVs as $komp)
		                      {
		                          ...
		                          $anrechnung->getAnrechnungPrestudent($prestudent_id, $row_tree->lehrveranstaltung_id, $komp);
