...
class LvPlanCSVDownload extends Auth_Controller
{
	...
	/**
	 * Serves an authenticated CSV file download.
	 *
	 * @return void
	 */
	public function index()
	{
		...
		$this->load->library('LvPlanCSVLib');
		$content = $this->lvplancsvlib->getContent(
			$uid,
			$begin,
			$ende,
			$type,
	...
	/**
	 * @return string|null Requested CSV target.
	 */
	private function getTarget()
	{
		$target = $this->input->get('target', true);

		if ($target !== null && $target !== 'outlook')
			show_error('Ungueltiger Parameter: target', 400);
