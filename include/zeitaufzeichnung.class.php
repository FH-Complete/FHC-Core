...
class zeitaufzeichnung extends basis_db
{
	...
	public function save()
	{
		...
		{
			$check_qry = "SELECT count(*) from campus.tbl_zeitaufzeichnung where uid=".$this->db_add_param($this->uid)." and aktivitaet_kurzbz != 'DienstreiseMT' and (start < ".$this->db_add_param($this->ende)." or ende > ".$this->db_add_param($this->start).")";
			if($this->db_query($check_qry))
			{
				if($row = $this->db_fetch_object())
				{
