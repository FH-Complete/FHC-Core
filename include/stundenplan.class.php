...
include_once('lehrstunde.class.php');
include_once('ferien.class.php');
...
class stundenplan
{
	...
	var $pers_titel;	// @brief Titel der Person
	...
	function stundenplan($type,$conn)
	{
		$this->type=$type;
		$this->conn=$conn;
	 ...
	 *
	 * @return einheit_kurzbz
	 *
	 */
	function load_data($type, $uid, $ort_kurzbz=NULL, $studiengang_kz=NULL, $sem=NULL, $ver=NULL, $grp=NULL, $einheit_kurzbz=NULL)
	{
		///////////////////////////////////////////////////////////////////////
		// Parameter Checken
		...
		elseif ($type=='einheit')
			$this->einheit_kurzbz=$einheit_kurzbz;
		...
		if ($this->type=='student' || $this->type=='lektor')
		{
			...
			if ($this->type=='student')
				$sql_query="SELECT uid, titel, nachname, vornamen, studiengang_kz, semester, verband, gruppe FROM vw_student WHERE uid='$this->pers_uid'";
			else
				$sql_query="SELECT uid, titel, nachname, vornamen FROM vw_lektor WHERE uid='$this->pers_uid'";
			//echo $sql_query;
			...
			$this->pers_titel=pg_result($result,0,'"titel"');
		...
		$sql_query="SELECT stunde, beginn, ende FROM tbl_stunde ORDER BY stunde";
		if(!$this->stunde=pg_exec($this->conn, $sql_query))
			die(pg_last_error($this->conn));

		// Studiensemesterdaten ermitteln
		$sql_query="SELECT * FROM tbl_studiensemester WHERE now()<ende ORDER BY start LIMIT 2";
		if(!$result=pg_exec($this->conn, $sql_query))
			die(pg_last_error($this->conn));
		else
		{
	...
	function load_week($datum, $stpl_view='stundenplan')
	{
		...
		$anz=$this->wochenplan->load_lehrstunden($this->type,$this->datum_begin,$this->datum_end,$this->pers_uid,$this->ort_kurzbz,$this->stg_kz,$this->sem,$this->ver,$this->grp,$this->einheit_kurzbz, $stpl_view);
		if ($anz<0)
		{
		...
		for($i=0;$i<$anz;$i++)
		{
			...
			$this->std_plan[$tag][$stunde][$idx]->einheit_kurzbz=$this->wochenplan->lehrstunden[$i]->einheit_kurzbz;
	...
	function draw_header()
	{
		...
		if ($this->type=='student' || $this->type=='lektor')
			echo '<strong>Person: </strong>'.$this->pers_titel.' '.$this->pers_vornamen.' '.$this->pers_nachname.' - '.$this->pers_uid.'<br>';
		if ($this->type=='student' || $this->type=='verband')
		{
		...
		//Kalender
		$this->kal_link.='&pers_uid='.$this->pers_uid.'&ort_kurzbz='.$this->ort_kurzbz.'&stg_kz='.$this->stg_kz.'&sem='.$this->sem.'&ver='.$this->ver.'&grp='.$this->grp.'&einheit_kurzbz='.$this->einheit_kurzbz;
			...
			for ($k=0; $k<$num_rows_stunde; $k++)
			{
				...
				if (isset($this->std_plan[$i][$j][0]->lehrfach))
				{
					...
					foreach ($this->std_plan[$i][$j] as $lehrstunde)
					{
						...
						if (count($lehrstunde->einheit_kurzbz)>0)
							$lvb=$lehrstunde->einheit_kurzbz;
						$lehrverband[]=$lvb;
						// Lehrfach
		...
		for ($i=1; $i<7; $i++)
		{
			...
			for ($k=0; $k<$num_rows_stunde; $k++)
			{
				...
				echo '<vbox style="border:1px solid black; background-color:'.$bgcolor.'"
					...
					grp="'.$this->grp.'" einheit="'.$this->einheit_kurzbz.'"
	...
	function load_stpl_search($datum,$stpl_id,$db_stpl_table, $block=1)
	{
		...
		$this->datum_end=date("Y-m-d",$this->datum_end);
		// Stundentafel abfragen
		$sql_query='SELECT min(stunde),max(stunde)FROM tbl_stunde';
		if(!$result_stunde=pg_exec($this->conn, $sql_query))
			die(pg_last_error($this->conn));
		...
		foreach ($lehrverband as $l)
		{
			...
			//if ($einheiten=='')
			//	$lvb.=' AND einheit_kurzbz IS NULL';
			$lvb.=')';
		}
		$lvb=substr($lvb,3);
	...
	function load_lva_search($datum,$lva_id,$db_stpl_table,$type)
	{
		...
		for ($i=0;$i<$num_rows_lva;$i++)
		{
			...
			$raumtyp[$i]=$row->raumtyp;
			$raumtypalt[$i]=$row->raumtypalternativ;
			if ($row->einheit_kurzbz!=null && $row->einheit_kurzbz!='')
				$einheit[$i]=$row->einheit_kurzbz;
			$lehrverband[$i]->stg_kz=$row->studiengang_kz;
			$lehrverband[$i]->sem=$row->semester;
			$lehrverband[$i]->ver=$row->verband;
			$lehrverband[$i]->grp=$row->gruppe;
