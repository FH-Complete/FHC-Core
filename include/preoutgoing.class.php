...
class preoutgoing extends basis_db
{
	  ...
	  */
	  public function getAll()
	  {
		...
		while($row = $this->db_fetch_object())
		{
			...
			$preoutgoing->prestudent_id = $row->prestudent_id;
	  ...
	  /**
	 * Laedt die Preoutgoing Datensätze mit der uebergebenen prestudent_id es wird immer der neueste zurückgegeben
	 * @param  $prestudent_id prestudent_id des zu ladenden Studenten
	 * @return true wenn ok, false im Fehlerfall
	 */
	public function loadPrestudent_id($prestudent_id)
	{
		//Pruefen ob id eine gueltige Zahl ist
		if(!is_numeric($prestudent_id))
		{
			$this->errormsg = 'Ungültige prestudent_id';
			return false;
		}

		//Daten aus der Datenbank lesen
		$qry = "SELECT * FROM public.tbl_preoutgoing WHERE prestudent_id=".$this->db_add_param($prestudent_id, FHC_INTEGER).' ORDER BY insertamum DESC LIMIT 1;';
	...
	public function save()
	{
		if($this->new)
		{
			...
			$qry='BEGIN;INSERT INTO public.tbl_preoutgoing (prestudent_id, dauer_von, dauer_bis, ansprechperson, bachelorarbeit, masterarbeit, projektarbeittitel,
	 ...
	 * @param  $preoutgoing_id preoutgoing_id des zu ladenden Outgoings
	 * @return true wenn ok, false im Fehlerfall
	 */
	...
	*/
	public function getOutgoingFilter($name ='', $von ='', $bis='', $status='')
	{
		$qry ="SELECT distinct(pre.preoutgoing_id), person.vorname, person.nachname, pre.* FROM public.tbl_preoutgoing pre
			...
			JOIN public.tbl_prestudent benutzer USING(prestudent_id)
