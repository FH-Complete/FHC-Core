...
class lvgesamtnote extends basis_db
{
	...
	public function save($new=null)
	{
		...
		if($new)
		{
			...
			$qry='INSERT INTO campus.tbl_lvgesamtnote (lehrveranstaltung_id, prestudent_id, studiensemester_kurzbz,
		...
		else
		{
			$qry='UPDATE campus.tbl_lvgesamtnote SET '.
				...
				'AND prestudent_id='.$this->db_add_param($this->prestudent_id).' '.
	 ...
	 */
	public function getLvGesamtNoten($lehrveranstaltung_id, $prestudent_id, $studiensemester_kurzbz)
	{
		$qry = "SELECT
					tbl_lvgesamtnote.*,
		...
		if($prestudent_id!=null)
			$qry.=" AND tbl_lvgesamtnote.prestudent_id=".$this->db_add_param($prestudent_id, FHC_INTEGER);
