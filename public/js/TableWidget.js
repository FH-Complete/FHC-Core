...
var FHC_TableWidget = {
  ...
  _getTables: function (renderFunction) {
    var tableWidgetUniqueIdArray =
      FHC_TableWidget._getTableWidgetUniqueIdArray();

    for (
      var tableWidgetsCounter = 0;
      tableWidgetsCounter < tableWidgetUniqueIdArray.length;
      tableWidgetsCounter++
    ) {
      FHC_AjaxClient.ajaxCallGet(
        ...
        {
          tableUniqueId:
            FHC_TableWidget._getTableUniqueIdPrefix() +
            "/" +
            tableWidgetUniqueIdArray[tableWidgetsCounter],
        },
        {
  ...
  _turnOnEvents: function (tableWidgetDiv) {
    ...
    if (FHC_TableWidget._datasetRepresentation == DATASET_REP_TABULATOR) {
      ...
      tableWidgetDiv.find("#download-csv").on("click", function () {
        ...
        tableWidgetDiv
          .find("#tableWidgetTabulator")
          .tabulator("download", "csv", "data.csv", { bom: true });
      });
  ...
  _renderDatasetTablesorter: function (tableWidgetDiv, data) {
    if (
      data.hasOwnProperty("checkboxes") &&
      data.checkboxes != null &&
      data.checkboxes.trim() != ""
    ) {
      tableWidgetDiv
        .find("#tableWidgetTableDataset > thead > tr")
        .append("<th data-filter='false' title='Select'>Select</th>");
    }
    ...
    for (var i = 0; i < arrayFieldsToDisplay.length; i++) {
      ...
      tableWidgetDiv
        .find("#tableWidgetTableDataset > thead > tr")
        .append("<th title='" + columnName + "'>" + columnName + "</th>");
    }

    if (
      data.hasOwnProperty("additionalColumns") &&
      $.isArray(data.additionalColumns)
    ) {
    ...
    if (arrayFieldsToDisplay.length > 0) {
      if (data.hasOwnProperty("dataset") && $.isArray(data.dataset)) {
        if (data.checkboxes != null && data.checkboxes != "") {
          ...
          if (
            typeof tableWidgetDiv.find("#tableWidgetTableDataset")
              .checkboxes === "function"
          )
            tableWidgetDiv
              .find("#tableWidgetTableDataset")
        ...
        if (data.dataset.length == 0) {
          ...
          strHtml += "</td></tr>";
          tableWidgetDiv
            .find("#tableWidgetTableDataset > tbody")
            .append(strHtml);
        } else {
  ...
  _enableTableSorter: function (tableWidgetDiv) {
    var tableWidgetTablesorter = tableWidgetDiv.find(
      "#tableWidgetTableDataset"
    );

    // Checks if the table contains data (rows)
    if (
      tableWidgetTablesorter.find("tbody:empty").length == 0 &&
  ...
  _renderDatasetPivotUI: function (tableWidgetDiv, data) {
    ...
    if (arrayFieldsToDisplay.length > 0) {
      ...
      if (data.hasOwnProperty("dataset") && $.isArray(data.dataset)) {
        ...
        // Renders the pivotUI
        tableWidgetDiv
          .find("#tableWidgetPivotUI")
          .pivotUI(pivotUIData, options);
      }
    }
  },
  ...
  _getFieldsToDisplay: function (data) {
    ...
    if (data.hasOwnProperty("fields") && $.isArray(data.fields)) {
      if (
        data.hasOwnProperty("columnsAliases") &&
        $.isArray(data.columnsAliases)
      ) {
  ...
  _renderDatasetTabulator: function (tableWidgetDiv, data) {
    ...
    var recordFieldsDefinitions =
      FHC_TableWidget._getRepresentationFieldsDefinitions(data);

    // Manipulation for the representation!
    var arrayTabulatorColumns = FHC_TableWidget._getTabulatorColumns(
      data,
    ...
    if (arrayTabulatorColumns.length > 0) {
      ...
      if (data.hasOwnProperty("dataset") && $.isArray(data.dataset)) {
        ...
        options.persistence =
          typeof options.persistence == "undefined"
            ? true
            : options.persistence; // enables persistence (default store in localStorage if available, else in cookie)
        options.persistenceID =
          typeof options.persistenceID == "undefined"
            ? data.tableUniqueId
            : options.persistenceID; // persistenceID to store persistence data seperately for multiple tables
        options.movableColumns =
          typeof options.movableColumns == "undefined"
            ? true
            : options.movableColumns; // allows changing column order
     ...
     */
    if (
      typeof options.tableWidgetFooter != "undefined" &&
      options.tableWidgetFooter != null
    ) {
      var tabulatorFooterHTML = _renderTabulatorFooterHTML(
        options.tableWidgetFooter
      );
  ...
  _getTabulatorColumns: function (data, recordFieldsDefinitions) {
      ...
      for (var sfc = 0; sfc < data.fields.length; sfc++) {
        for (var fc = 0; fc < data.fields.length; fc++) {
          if (data.fields[sfc] == data.fields[fc]) {
            ...
            if (
              recordFieldsDefinitions != null &&
              recordFieldsDefinitions.hasOwnProperty(data.fields[sfc])
            ) {
    ...
    if (
      ...
      $.each(data.additionalColumns, function (i, additionalColumn) {
        ...
        if (
          recordFieldsDefinitions != null &&
          recordFieldsDefinitions.hasOwnProperty(additionalColumn)
        ) {
  ...
  _getRepresentationOptions: function (data) {
    ...
    if (
      data.hasOwnProperty("datasetRepresentationOptions") &&
      data.datasetRepresentationOptions != ""
    ) {
  ...
  _getRepresentationFieldsDefinitions: function (data) {
    ...
    if (
      data.hasOwnProperty("datasetRepresentationFieldsDefinitions") &&
      data.datasetRepresentationFieldsDefinitions != ""
    ) {
      var tmpFDefs = eval(
        "(" + data.datasetRepresentationFieldsDefinitions + ")"
      ); // and converts them from string to javascript code
...
function _func_rowSelectionChanged(data, rows) {
  $("#number-selected").html(
    "Ausgewählte Zeilen: <strong>" + rows.length + "</strong>"
  );
}
  ...
  tabulatorHeaderHTML += `<div class="btn-toolbar ${
    _bootstrapVersion == 3 ? "pull-right" : "float-end"
		...
		<div class="btn-group" role="group">
			<!-- CSV Download button -->
			<button id="download-csv" class="btn ${
        _bootstrapVersion == 3 ? "btn-default" : "btn-outline-secondary"
      }" type="button" 
...
function _renderTabulatorHeaderCollapseHTML(
  tableWidgetDiv,
  arrayTabulatorColumns
) {
  var tableUniqueId = tableWidgetDiv.attr("tableUniqueId");
  ...
  tableWidgetDiv
    ...
    .forEach(function (column) {
      ...
      var btn_select_col_selected = column.isVisible()
        ? "btn-select-col-selected"
        : "";

      // If certain columns should be excluded from the column picker (define them in a blacklist array)
      if (
        typeof tableWidgetBlacklistArray_columnUnselectable != "undefined" &&
        Array.isArray(tableWidgetBlacklistArray_columnUnselectable) &&
        tableWidgetBlacklistArray_columnUnselectable.length
      ) {
        if (
          $.inArray(field, tableWidgetBlacklistArray_columnUnselectable) < 0
        ) {
          tabulatorHeaderCollapseHTML += `<button type="button" style="flex:0 0 auto " class="btn ${
            _bootstrapVersion == 3
              ? "btn-default"
              : "btn-outline-secondary flex-shrink-1"
          } btn-sm btn-select-col ${btn_select_col_selected} " aria-pressed="true" id="btn-${field}" value="${field}">${title}</button>`;
        }
      }
  ...
  if (
    ...
    tabulatorFooterHTML += `<div class="btn-toolbar" role="toolbar">
          ...
          _bootstrapVersion == 3
            ...
            : "btn-outline-secondary float-start"
        } " type="button">
			 		${FHC_PhrasesLib.t("ui", "alleAuswaehlen")}
				</button>
				<button id="deselect-all" class="btn ${
          _bootstrapVersion == 3
            ? "btn-default pull-left"
            : "btn-outline-secondary float-start"
        } " type="button">
