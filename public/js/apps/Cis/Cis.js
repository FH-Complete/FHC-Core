...
import NewsAdministration from '../../components/Cis/Cms/NewsAdministration/NewsAdministration.js';
...
import RoomInformation, {
	DEFAULT_MODE_RAUMINFO_DESKTOP,
	DEFAULT_MODE_RAUMINFO_MOBILE,
} from '../../components/Cis/Mylv/RoomInformation.js';
...
const router = VueRouter.createRouter({
	...
	routes: [
		...
		// Redirect old links to new format
		{
			...
			redirect: (to) => {
				return {
					// redirect to longer Rauminfo url and map params
					name: 'RoomInformation',
					params: {
						// in this case always populate other params since they are not optional
		...
		},
		{
			...
			component: RoomInformation,
			props: (route) => {
				// validate and set mode/focus date if for some reason missing
				...
				const mode =
					route.params.mode &&
					validModes.includes(
						route.params.mode.charAt(0).toUpperCase() +
							route.params.mode.slice(1).toLowerCase(),
					)
						? route.params.mode.charAt(0).toUpperCase() +
							route.params.mode.slice(1).toLowerCase()
						: isMobile
				...
				const focus_date = !isNaN(d)
					? route.params.focus_date
				...
				if (!to.params.mode || !to.params.focus_date) {
					next({
						...
						params: {
							...
							focus_date:
								to.params.focus_date || new Date().toISOString().split('T')[0],
							ort_kurzbz: route.params.ort_kurzbz,
						},
					});
				} else {
		...
		// Redirect old links to new format
		{
			...
			redirect(to) {
				...
				return {
					// redirect to longer LvPlan url and map params
					name: 'LvPlan',
					params: {
						mode,
						focus_date,
...
const app = Vue.createApp({
	...
	provide() {
		return {
			// provide injectable & watchable language property
