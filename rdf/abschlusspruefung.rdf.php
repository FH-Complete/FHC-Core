...
if($db->db_query($qry))
	...
	function draw_content_xml($row)
	{
		...
		$qry = "SELECT titel as themenbereich, ende, projektarbeit_id, note FROM lehre.tbl_projektarbeit a WHERE student_uid='$student->uid' AND (projekttyp_kurzbz='Bachelor' OR projekttyp_kurzbz='Diplom') ORDER BY beginn DESC, projektarbeit_id DESC LIMIT 2";
		$themenbereich='';
		$datum_projekt='';
		$betreuer = '';
		$betreuer_2 = '';
