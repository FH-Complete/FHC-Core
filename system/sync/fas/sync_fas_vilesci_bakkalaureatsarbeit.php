	...
	while($row = pg_fetch_object($result))
	{
		...
		if(!$error)
		{
			...
			if(!$error)
			{
				...
				if(!$error)
				{
					$qry2="SELECT * FROM lehre.tbl_lehreinheit WHERE lehrveranstaltung_id='".$lehreinheitlehrveranstaltung_id."' AND anmerkung='Bachelorarbeit' AND ext_id='".$row->bakkalaureatsarbeit_pk."';";
