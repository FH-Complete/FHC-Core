...
// **************************************
	...
	function getNoten4Studiengang($conn_fas, $studiengang_pk)
	{
		$fasnoten_arr = array();
		$mehrfach_arr = array();
		...
		$sqlstr = "SELECT note.note_pk, 
						...
						lehrveranstaltung.notenlektor_fk,
						note.student_fk
					FROM note, benutzer, lehrveranstaltung, student
					WHERE note.creationuser = benutzer.benutzer_pk AND
						note.lehrveranstaltung_fk = lehrveranstaltung.lehrveranstaltung_pk AND
						note.student_fk = student.student_pk AND
						student.studiengang_fk = '".$studiengang_pk."'
					ORDER BY note.student_fk ASC, note.lehrveranstaltung_fk ASC, note.datum DESC, note.creationdate DESC";
		if($result = pg_query($conn_fas, $sqlstr))
		{
			//$anzahl_quelle = pg_num_rows($result);
			while($row = pg_fetch_object($result))
			{
				...
				if (key_exists($row->student_fk, $fasnoten_arr))
				{			
					if (key_exists($row->lehrveranstaltung_fk,$fasnoten_arr[$row->student_fk]))
					{
						$bemerkung_history = "Note am ".$row->datum.": ".$row->note;
						$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["main"] = 0;
					}
					else
					{
						$mehrfach_arr[$row->student_fk][$row->lehrveranstaltung_fk]["main"] = $row->note_pk;
						$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["main"] = 1;
					}					
				...
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["note"] = $row->note;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["status"] = $row->status;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["creationdate"] = $row->creationdate;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["creationuser_name"] = $row->name;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["name"] = $row->name;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["studiensemester_fk"] = $row->studiensemester_fk;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["bemerkung"] = $row->bemerkung;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["datum"] = $row->datum;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["notenlektor_fk"] = $row->notenlektor_fk;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["student_fk"] = $row->student_fk;
				$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$row->note_pk]["ext_id"] = $row->note_pk;
				if ($bemerkung_history != "")
					$fasnoten_arr[$row->student_fk][$row->lehrveranstaltung_fk][$mehrfach_arr[$row->student_fk][$row->lehrveranstaltung_fk]["main"]]["bemerkung"] .= "\n".$bemerkung_history;
			}
	...
	*/
	$getstr = "";	
	$sqlstr = "select * from studiengang";
	if (isset($_REQUEST["stg_von"]))		
		$getstr .= " studiengang_pk >= '".$_REQUEST["stg_von"]."'";
	if (isset($_REQUEST["stg_bis"]))
	{
		if ($getstr != "")
			$getstr .= " AND";		
		$getstr .= " studiengang_pk <= '".$_REQUEST["stg_bis"]."'";
	}
	if ($getstr != "")
		$getstr = " WHERE".$getstr;
		
	$sqlstr = $sqlstr.$getstr." order by studiengang_pk";
	//echo $sqlstr;
	...
	if($result = pg_query($conn_fas, $sqlstr))
	{
		...
		while($row = pg_fetch_object($result))
		{
			...
			{
				$text .=  "<b>*** Student: ".$studkey."</b><br>";
				$anz_students_processed++;
								
				$lvkeys_arr = array_keys($fasnoten_arr[$studkey]);
				foreach ($lvkeys_arr as $lvkey)
				{
					$idkeys_arr = array_keys($fasnoten_arr[$studkey][$lvkey]);
					foreach ($idkeys_arr as $idkey)
					{
						...
						if (key_exists($fasnoten_arr[$studkey][$lvkey][$idkey]["student_fk"],$studenten_arr))
							$student_uid = $studenten_arr[$fasnoten_arr[$studkey][$lvkey][$idkey]["student_fk"]];
						else
							$student_uid = "FEHLT";
	
						$mitarbeiter_uid = $mitarbeiter_arr[$fasnoten_arr[$studkey][$lvkey][$idkey]["notenlektor_fk"]];
						$note = $fasnoten_arr[$studkey][$lvkey][$idkey]["note"];
						$pruefungstyp_kurzbz = getPruefungstyp($fasnoten_arr[$studkey][$lvkey][$idkey]["status"]);
						$datum = $fasnoten_arr[$studkey][$lvkey][$idkey]["datum"];
						$anmerkung = $fasnoten_arr[$studkey][$lvkey][$idkey]["bemerkung"];
						$insertamum = $fasnoten_arr[$studkey][$lvkey][$idkey]["creationdate"];
						$insertvon = $fasnoten_arr[$studkey][$lvkey][$idkey]["creationuser_name"];
						$updatevon = "sync";
						$ext_id = $idkey;
	
						$zeugnistabeintrag = $fasnoten_arr[$studkey][$lvkey][$idkey]["main"];
