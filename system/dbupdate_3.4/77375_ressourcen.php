...
$result = $db->db_query("SELECT 1 FROM information_schema.columns WHERE table_schema = 'lehre' AND table_name = 'tbl_kalender' AND column_name = 'eindeutige_kalender_gruppen_id'");
if($db->db_num_rows($result) === 0)
{

	$qry = "ALTER TABLE lehre.tbl_kalender
			ADD COLUMN IF NOT EXISTS eindeutige_kalender_gruppen_id UUID";

	if(!$db->db_query($qry))
		echo '<strong>lehre.tbl_kalender: '.$db->db_last_error().'</strong><br>';
	else
		echo '<br>lehre.tbl_kalender column eindeutige_kalender_gruppen_id hinzugefuegt';
}
