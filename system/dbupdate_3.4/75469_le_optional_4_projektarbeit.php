...
// run the data migration and the constraint change only once.
// Target state: lehrveranstaltung_id NOT NULL, studiensemester_kurzbz NOT NULL, lehreinheit_id NULLable.
// The check returns a row as long as one of the three columns is not in the target state.
$state_check = "SELECT 1 FROM information_schema.columns
                WHERE table_schema = 'lehre' AND table_name = 'tbl_projektarbeit'
                AND ((column_name IN ('lehrveranstaltung_id', 'studiensemester_kurzbz') AND is_nullable = 'YES')
                     OR (column_name = 'lehreinheit_id' AND is_nullable = 'NO'))";

$migration_pending = false;

if ($result = $db->db_query($state_check)) {
	$migration_pending = ($db->db_num_rows($result) > 0);
} else {
	echo "<strong>State Check Error: ".$db->db_last_error()."</strong><br>";
}
