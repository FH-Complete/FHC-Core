...
describe("Noten API - Prüfungstermin (Schreibpfad)", () => {
	...
	it("bearbeitet die Prüfung mit der übergebenen pruefung_id", () => {
		...
		seedPruefung(ctx, student, { note: ctx.notes.entschuldigt, datum: excusedDate, typ: "Termin2" })
			...
			.then((data) => {
				...
				expect(String(graded.note), "the untargeted attempt keeps its grade").to.eq(String(ctx.gradeNotes[0]));
	...
	it("schreibt die LV-Note aus der Note des Antritts", function () {
		...
		addPruefung(ctx, student, { note: ctx.gradeNotes[1], datum: attemptDate(ctx, 1) })
			...
			.then((row) => {
				expect(String(row.note), "the LV note follows the newest attempt").to.eq(String(ctx.gradeNotes[1]));
			});
	});
	...
	describe("Freigabe nach einem neuen Termin", () => {
		...
		const neuerTerminNachFreigabe = (student) => {
			...
			return readLvGesamtnoteViaDb(ctx, student.uid);
		};
	...
	it("legt die LV-Note an, wenn ein Studierender noch keine hat", () => {
		...
		notenApi
			...
			.then((row) => {
				...
				expect(String(row.note), "carries the Prüfung's note").to.eq(String(ctx.notes.nochNichtEingetragen));
			});
	});
});
